...
used to garantee a thread safe execution in the cases where
...
be resized) and the new size for it. If the new size is valid, the function reallocs memory for the variable and returns the pointer.
...
and \texttt{freefinix.c}\footnote{Serpent has the function \texttt{FreeFinix()} which
  is used exclusivelly for multiphysics calculations using the FINIX code. This function
