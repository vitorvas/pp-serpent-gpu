...
program resumes its execution after the and of the loop. This construction only
applied to the marked loop. The fact of having the loop variable private to each
thread avoid the burden of taking care of race conditions to the programmer.

\begin{itemize}
\item \texttt{\#pragma omp atomic}: $10$ times.
\item \texttt{\#pragma omp parallel private(...)}: $45$ times.
\item \texttt{\#pragma omp for}: $25$ times.
\item \texttt{\#pragma omp critical}: $8$ times.
\item \texttt{\#pragma omp parallel for}: $5$ times.
\item \texttt{\#pragma omp for schedule (dynamic)}: $7$ times.
\item \texttt{\#pragma omp critical (event)}: $2$ times.
\item \texttt{\#pragma omp critical (eventblock)}: $2$ times.
\end{itemize}

The \texttt{\#pragma omp for} is used to split the loop iterations among threads.
The use of it followed by the clause \texttt{schedule} controls its behavior. This
clause affect how the loop iterations are mapped onto threads. If \texttt{schedule(static[,chunk])}
...
compiler dispatch the decisions to the operating system running Serpent.


An example of its use in Serpent is...
